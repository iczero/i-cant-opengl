...
void NBody::read_bodies_from_csv(const char* file_name)
{
    ...
    for (; getline(csv_file, line);) {
        ...
        float x_coordinate = stod(x_coordinate_string);
        ...
        float y_coordinate = stod(y_coordinate_string);
        ...
        float z_coordinate = stod(z_coordinate_string);
        ...
        float x_velocity = stod(x_velocity_string);
        ...
        float y_velocity = stod(y_velocity_string);
        ...
        float z_velocity = stod(z_velocity_string);
        ...
        float x_force = stod(x_force_string);
        ...
        float y_force = stod(y_force_string);
        ...
        float z_force = stod(z_force_string);
        ...
        float radius = stod(radius_string);
        radius_string.clear();
...
{
    srand((unsigned long long)malloc(1)); // Lol
    for (int i=0; i < count; i++) { // was 10000
        Vec3 position_vec = Vec3 {
            (float)(rand() % 10000) / (float)10 - 500,
            (float)(rand() % 10000) / (float)10 - 500,
            (float)(rand() % 10000) / (float)10 - 500 };
        ...
        std::cout << position_vec.x
                  ...
                  << ',' << force_vec.y
                  << ',' << force_vec.z
                  << ',' << (float)(rand() % 1000) / (float)10 - 50
                  << ',' << (float)(rand() % 1000) / (float)10 - 50 << '\n';
        */

        // m_bodies->insert(new OctreeNode { position_vec, velocity_vec, force_vec, (float)(rand() % 1000) / (float)10 - 50, (float)(rand() % 1000) / (float)10 - 50 });
        m_bodies->insert(new OctreeNode { position_vec, velocity_vec, force_vec, (float)(rand() % 1000) / (float)10 - 50, (float)(rand() % 1000) / (float)10 - 50 });
    }
}
...
float mass_from_body_cluster(Octree* tree) // Probably segfaulty
{
    float sum = 0;
    for (int i=0; i < 8; i++) {
        if (!tree->getChildren()[i])
...
Vec3 NBody::calculate_force(OctreeNode* node_a, OctreeNode&& node_b)
{
    ...
    return Vec3 { (6.674e-11f * node_a->mass * node_b.mass) / ((node_a->position.x - node_b.position.x) - (node_a->radius + node_b.radius)),
                  (6.674e-11f * node_a->mass * node_b.mass) / ((node_a->position.y - node_b.position.y) - (node_a->radius + node_b.radius)),
                  (6.674e-11f * node_a->mass * node_b.mass) / ((node_a->position.z - node_b.position.z) - (node_a->radius + node_b.radius)) };
}
Vec3 NBody::calculate_force(OctreeNode* node_a, OctreeNode* node_b)
{
    if (!node_a || !node_b)
...
void NBody::calculate_force_and_acceleration(Octree* tree, OctreeNode* node)
{
    ...
    float octant_width = 1000 / octant; // 1000 is the hardcoded width of the head octant. Change this to be setable by the user?
    Vec3 center_of_mass = calculate_center_of_mass(tree);
    Vec3 distance = center_of_mass - node->position;

    if (octant_width / distance.norm() < 0.5) { // then can be treated as a single body
...
void NBody::update_position(OctreeNode* node)
{
    float acceleration_x = (node->force.x / node->mass);
    float acceleration_y = (node->force.y / node->mass);
    float acceleration_z = (node->force.z / node->mass);
    node->position.x += acceleration_x * acceleration_x * ((acceleration_x < 0) ? -1 : 1);
    node->position.y += acceleration_y * acceleration_y * ((acceleration_y < 0) ? -1 : 1);
    node->position.z += acceleration_z * acceleration_z * ((acceleration_z < 0) ? -1 : 1);
}
